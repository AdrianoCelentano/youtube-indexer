...
class YouTubeApiClient(
    ...
    /**
     * One page of the signed-in user's subscriptions.
     *
     * Costs 1 quota unit per page of up to 50, so even hundreds of subscriptions are
     * negligible next to the videos themselves.
     */
    suspend fun subscriptionPage(pageToken: String? = null): Page<String> {
        val response =
            request<ListResponse<SubscriptionDto>>("subscriptions") {
                parameter("part", "snippet")
                parameter("mine", "true")
                parameter("maxResults", MAX_IDS_PER_REQUEST)
                pageToken?.let { parameter("pageToken", it) }
            }

        return Page(
            items = response.items.mapNotNull { it.snippet?.resourceId?.channelId },
            nextPageToken = response.nextPageToken,
        )
    }

    /**
     * Resolves channel ids to their titles and uploads playlists, up to
     * [MAX_IDS_PER_REQUEST] at a time.
     *
     * Batched for the same reason video details are: one call for fifty channels rather
     * than fifty calls.
     */
    suspend fun channels(channelIds: List<String>): List<SubscribedChannel> {
        require(channelIds.size <= MAX_IDS_PER_REQUEST) {
            "channels.list accepts at most $MAX_IDS_PER_REQUEST ids, got ${channelIds.size}"
        }
        ...
        val response =
            request<ListResponse<ChannelDto>>("channels") {
                ...
                parameter("id", channelIds.joinToString(","))
                parameter("maxResults", channelIds.size)
            }

        return response.items.mapNotNull { dto ->
            // A channel with no uploads playlist has nothing to index; skipping it is
            // correct rather than an error, since one dead subscription must not fail
            // the whole sync.
            dto.contentDetails?.relatedPlaylists?.uploads?.let { uploads ->
                SubscribedChannel(
                    channelId = dto.id,
                    title = dto.snippet?.title.orEmpty(),
